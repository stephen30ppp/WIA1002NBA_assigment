...
class Graph {
    ...
    private void addPath(String from, String to, double distance) {
        ...
        toCity.addNeighbor(fromCity, distance); // Assuming bidirectional paths
    }

    public List<String> findPaths(String from, String to) {
        List<String> paths = new ArrayList<>();
        City start = getCity(from);
        City end = getCity(to);
        List<City> path = new ArrayList<>();
        path.add(start);
        findPathsUtil(start, end, paths, path, 0);
        return paths;
    }

    private void findPathsUtil(City current, City end, List<String> paths, List<City> path, double distance) {
        ...
        if (current == end) {
            ...
            for (int i = 0; i < neighbors.size(); i++) {
                ...
                if (!neighbor.isVisited()) {
                    path.add(neighbor);
                    findPathsUtil(neighbor, end, paths, path, distance + dist);
    ...
    public String findShortestPath(String from, String to) {
        List<String> paths = findPaths(from, to);
